/**
 * LoginForm.jsx — Thin wrapper kept for backward compatibility.
 *
 * The full login UI now lives in LoginOrRegister.jsx (the /masuk page).
 * This component is no longer the primary rendering path but is kept so
 * any remaining import doesn't break the build.
 *
 * It delegates to the AuthContext `login` function (replaces old `signIn`).
 */
...
export function LoginForm({ onSuccess }) {
  ...
  const canSubmit = !!parsed && password.length >= 1 && !loading;
  ...
  return (
    <form className="flex flex-col gap-4" onSubmit={handleSubmit} noValidate>
      ...
      >
        {loading ? <><Spinner /><span>Memproses...</span></> : "Masuk"}
      </button>
    </form>
  );
}
